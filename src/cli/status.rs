...
fn detail(project: &Project, state: &RunState) -> Result<()> {
    ...
    if let Some(subagents) = &state.subagents {
        if !subagents.is_empty() {
            println!("\nSubagents");
            for s in subagents {
                let files = if s.files.is_empty() { "(none)".to_string() } else { s.files.iter().map(|p| p.display().to_string()).collect::<Vec<_>>().join(", ") };
                let cost = s.cost_usd.map(|c| format!(" ${:.2}", c)).unwrap_or_default();
                println!("  {:<12} {:<10} {}{}  {}", s.id, s.status.to_string(), truncate(&s.task, 30), cost, files);
            }
        }
    }
